...
function vitePluginLocalLookupFunction(): Plugin {
  ...
  return {
    name: "local-player-lookup-function",
    configureServer(server: ViteDevServer) {
      server.middlewares.use("/api/query", async (req, res, next) => {
        ...
        let requestUrl: URL;
        try {
          requestUrl = new URL(req.url || "/api/query", "http://localhost");
        } catch {
          res.writeHead(400, { "Content-Type": "application/json; charset=utf-8" });
          res.end(JSON.stringify({ success: false, error: "Malformed request URL." }));
          return;
        }
        ...
        try {
          const result = await (await loadHandler())({
